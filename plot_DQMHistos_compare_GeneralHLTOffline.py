...
def plot_canvas(target, reference, target_legend, reference_legend, output, output_extensions):
    ...
    if not ratio:
        ...
        h_refe.SetStats(0)
        h_refe.Draw(opt_draw)
        h_targ.Draw(opt_draw+',same')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t-leg', dest='target_legend', required=True, action='store', default='',
    ...
    parser.add_argument('--only-keys', dest='only_keys', nargs='+', default=['/HLT/', '/GeneralHLTOffline/'],
